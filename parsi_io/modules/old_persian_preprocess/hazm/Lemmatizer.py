...
class Lemmatizer(object):
    ...
    def conjugations(self, verb):
        ...
        aux_present = self.add_ya(present)
        past_ends = ['م', 'ی', '', 'یم', 'ید', 'ند']
    ...
    # def not_verbs(self, verbs: List[str]):
    #     first_verb = verbs[0]
    #     if first_verb.startswith('ا') or first_verb.startswith('آ'):
    #
    #     return list(map(lambda item: 'ن' + item, verbs))

    def add_ya(self, term: str, to_first=True):
        if to_first:
            term = 'ی' + term[1:] if term.startswith('ا') or term.startswith('آ') else term
        else:
            term = term + 'ی' if not term.endswith('ی') else term
        return term
